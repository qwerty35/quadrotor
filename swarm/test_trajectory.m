function [xtraj, ttraj, terminate_cond] = test_trajectory(start, stop, map, path, makespan, vis)
...
trajhandle    = @trajectory_swarm_generator;
...
trajhandle([], [], map, path, makespan, params);
...
% Initialize quadEOM_CO
quadEOM_CO([], [], [], controlhandle, trajhandle, params, map);
warning('off')
%% ************************* RUN SIMULATION *************************
OUTPUT_TO_VIDEO = 1;
...
%         [tsave, xsave] = ode15s(@(t,s) quadEOM_CO(t, s, qn, controlhandle, trajhandle, params, map), timeint, x{qn});
        x{qn} = xsave(end, :)';
        % Save to traj
